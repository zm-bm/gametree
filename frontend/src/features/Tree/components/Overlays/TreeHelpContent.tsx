...
import { TreeDimensionsContext } from "@/features/Tree/context/TreeDimensionsContext";
...
export const TreeHelpContent = () => (
  <>
    <section className="text-sm text-lightmode-800/85 dark:text-darkmode-200/85">
      Explore the game tree from the current board position. The tree shows common continuations and results, while the pinned board HUD stays available for playing through the line.
    </section>

    <section className="pt-1 border-t gt-divider-surface">
      <h3 className="text-base font-semibold mb-2">Board HUD</h3>
      <ul className="space-y-1.5 text-sm text-lightmode-800/90 dark:text-darkmode-200/90">
        <li>The board in the top-left always shows the current position and accepts legal moves.</li>
        <li>Use the board to enter a move even when that continuation is not visible in the tree yet.</li>
        <li>Flip board changes orientation. On mobile, expand board opens a larger temporary board view.</li>
        <li>The summary, engine, and theory buttons open optional HUD panels; close them when you want more tree space.</li>
      </ul>
    </section>
    ...
    <section className="pt-1 border-t gt-divider-surface">
      ...
      <ol className="space-y-1.5 text-sm text-lightmode-800/90 dark:text-darkmode-200/90 list-decimal pl-4">
        <li>Click a node to follow a visible move, or play a legal move directly on the board HUD.</li>
        ...
        <li>Open summary, engine, or theory panels when you need detail, then close them to return to the tree.</li>
