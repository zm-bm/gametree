...
describe('TreeHelp', () => {
  ...
  it('shows the first-run hint without auto-opening help', () => {
    ...
    expect(screen.getByText(
      'Click a move to follow it, or use the board HUD to play legal moves that are not visible yet.'
    )).toBeInTheDocument();
  ...
  it('opens from help button, dismisses the hint, and marks help as seen when closed', () => {
    ...
    expect(screen.queryByRole('note', { name: 'Tree explorer hint' })).not.toBeInTheDocument();
    expect(screen.getByTestId('help-modal-mock')).toBeInTheDocument();
    expect(screen.getByText('Board HUD')).toBeInTheDocument();
    expect(screen.getByText(
      'The board in the top-left always shows the current position and accepts legal moves.'
    )).toBeInTheDocument();
    expect(screen.getByText(
      'The summary, engine, and theory buttons open optional HUD panels; close them when you want more tree space.'
    )).toBeInTheDocument();
